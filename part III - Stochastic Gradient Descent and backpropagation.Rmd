...
Conclusion if we can't find minimum of afunction `f(x)` directly, we can always try checking where derivative `f'(x)` is equal to `0` (and for multiple solutions check which one is a minimum). That's great, but what to do when we can't solve the equasion `f'(x) == 0` ? No problem, we can always use very simple algorithm called **Gradient Descent**:
